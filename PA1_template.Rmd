...
meanStepsByDate   <- mean(StepsByDate)
medianStepsByDate <- median(StepsByDate)
```
The mean total number of steps taken per day is `r meanStepsByDate`.
The median total number of steps taken per day is `r medianStepsByDate`.
```{r Question1}
# Calculate total steps per day
StepsByDate <- tapply(X=data$steps, INDEX=data$date, FUN=sum, na.rm=T)
...
```
The data pattern shows people usually start exercising around 6:00AM every day with the peak around `r ActivityByInterval.maxDate`AM with `r ActivityByInterval.maxStep` steps.
```{r Question2 }
# Calculate total steps per day
ActivityByInterval <- aggregate(data$steps ~ format(data$DateTime, "%H:%M"), FUN=mean, na.rm=T)
names(ActivityByInterval) <- c("Hour_Minute", "Avg_Steps")
...
plot(x=timeHM_formatter(ActivityByInterval$Hour_Minute),
     ...
     xlab="5 Minute Daily Intervals", ylab="Mean Steps per Day",
     ylim=c(0,250))
